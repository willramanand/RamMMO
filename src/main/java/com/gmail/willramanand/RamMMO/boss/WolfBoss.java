...
public class WolfBoss extends MMOBoss {
  ...
  public WolfBoss(RamMMO plugin, Location loc) {
    ...
    this.bossWolf =
        loc.getBlock()
            .getWorld()
            .spawn(loc.getBlock().getLocation().add(0.5, 0.5, 0.5), Wolf.class);
  ...
  public void healthBar() {
    ...
    BossBar bossBar =
        plugin
            .getServer()
            .createBossBar(bossWolf.getCustomName(), BarColor.RED, BarStyle.SEGMENTED_6, flag);
    bossBar.setVisible(true);

    double progress =
        bossWolf.getHealth() / bossWolf.getAttribute(Attribute.GENERIC_MAX_HEALTH).getBaseValue();
  ...
  public void nearbyEntities() {
    Collection<Entity> nearbyEntities =
        bossWolf.getLocation().getWorld().getNearbyEntities(bossWolf.getLocation(), 10, 10, 10);
      ...
      public void run() {
        List<Entity> playersNearby =
            bossWolf
                .getLocation()
                .getWorld()
                .getNearbyEntities(bossWolf.getLocation(), 15, 15, 15)
                .stream()
