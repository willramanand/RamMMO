...
public class HealthListener implements Listener {
  ...
  public void onMove(PlayerMoveEvent event) {
    ...
    List<Entity> from =
        event.getFrom().getWorld().getNearbyEntities(event.getFrom(), 5, 5, 5).stream()
            .filter(
                ent ->
                    ent instanceof LivingEntity
                        && !(ent instanceof Boss
                            || ent instanceof Player
                            || ent instanceof MMOBoss
                            || ent instanceof ArmorStand))
            .toList();
    List<Entity> to =
        event.getFrom().getWorld().getNearbyEntities(event.getTo(), 5, 5, 5).stream()
            .filter(
                ent ->
                    ent instanceof LivingEntity
                        && !(ent instanceof Boss
    ...
    to.forEach(
        ent -> {
          ...
          String maxHealth =
              plugin
                  .getFormatter()
                  .format(entity.getAttribute(Attribute.GENERIC_MAX_HEALTH).getValue());
          ...
          if ((entity.getName().equalsIgnoreCase(mobName) || entity.getName().contains("❤"))
              && event.getPlayer().hasLineOfSight(entity)) {
            entity.setCustomName(
                ColorUtils.colorMessage(
                    mobName + "&8 | &c&l❤ &f" + currHealth + "&8/&f" + maxHealth));
            ...
            if (entity.getName().contains("☠")
              ...
              entity.setCustomName(
                  ColorUtils.colorMessage(
                      mobName + "&8 | &6&l☠ &f" + currHealth + "&8/&f" + maxHealth));
              entity.setCustomNameVisible(true);
              ...
              entity.setMetadata(
                  "Nickname", new FixedMetadataValue(plugin, entity.getCustomName()));
  ...
  public void onMobDamage(EntityDamageEvent event) {
    List<Entity> playersNearby =
        event
            .getEntity()
            .getLocation()
            .getWorld()
            .getNearbyEntities(event.getEntity().getLocation(), 5, 5, 5)
            .stream()
            .filter(player -> player instanceof Player)
            .toList();

    playersNearby.forEach(
        player -> {
          ...
          String currHealth =
              plugin
                  .getFormatter()
                  .format(
                      (entity.getHealth() - event.getFinalDamage() < 0)
                          ? 0
